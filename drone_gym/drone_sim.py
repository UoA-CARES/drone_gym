...
class DroneSim:
    def __init__(self, uri="udp://0.0.0.0:19850", simulation=True):
        ...
        self.position_deadband = 0.10

        # Velocity ramping for smooth transitions
        self.current_commanded_velocity = {"x": 0.0, "y": 0.0, "z": 0.0}
        self.max_velocity_change_rate = 100
        ...
        # Position tracking (from Gazebo via state estimate)
        ...
        self.position_update_rate = 0.0166666  # AAAAAAAAAAAAAAAAAAA 20Hz position update
        ...
        # aaaaaaaaaaaaaaaaaaaaaaa Debugging parameters
    ...
    def _start_threads_coordinated(self):
        ...
        if not self.hardware_ready_event.wait(timeout=60):
            print("[Drone] ERROR: Hardware failed to initialize in time")
            print("[Drone] Make sure Gazebo is running and the drone model is loaded")
