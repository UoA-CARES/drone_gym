...
class EvadePursuers2DParticle(DroneEnvironment):
    ...
    def __init__(
        ...
        n_pursuers: int = 1,
        pursuer_max_velocity: float = 0.30,
    ):
        ...
        self.pursuer_positions: List[List[float]] = [
            [0.0, 0.0, self.fixed_z] for _ in range(n_pursuers)
        ]
        self.pursuer_velocities: List[List[float]] = [
            [0.0, 0.0, 0.0] for _ in range(n_pursuers)
        ]
    ...
    def _run_gz_service(
        self, service: str, reqtype: str, reptype: str, req: str
    ) -> bool:
        cmd = [
            "gz",
            "service",
...
"""
    ...
    def _spawn_all_markers(self):
        for i in range(self.n_pursuers):
            ...
            if self._spawn_marker(i, pos[0], pos[1], pos[2]):
                ...
                self._markers_spawned[i] = self._set_marker_pose(
                    i, pos[0], pos[1], pos[2]
                )
    ...
    def _start_particle_thread(self):
        ...
        self._particle_thread = threading.Thread(
            target=self._particle_update_loop, daemon=True
        )
    ...
    def _compute_pursuer_velocity(
        self, pursuer_pos: List[float], evader_pos: List[float]
    ) -> List[float]:
    ...
    def _update_pursuer_velocities(self, evader_pos: List[float]):
        with self._particle_lock:
            for i in range(self.n_pursuers):
                vx, vy, _ = self._compute_pursuer_velocity(
                    self.pursuer_positions[i], evader_pos
                )
    ...
    def _sample_pursuer_spawn_positions(
        self, evader_pos: List[float]
    ) -> List[List[float]]:
        limit = self.xy_limit - self.spawn_margin
        ...
        for i in range(self.n_pursuers):
            ...
            for _ in range(200):
                ...
                y = float(np.random.uniform(-limit, limit))
                if (
                    math.sqrt((x - evader_pos[0]) ** 2 + (y - evader_pos[1]) ** 2)
            ...
            if not placed:
                ...
                targets.append(
                    [math.cos(angle) * limit, math.sin(angle) * limit, self.fixed_z]
                )
        return targets
    ...
    def reset(self, training: bool = True):
        ...
        if self._drone_is_dead(self.drone.get_position()):
            print(
                "[EvadePursuers2DParticle] EKF blow-up detected after parent reset — restarting drone"
            )
            self.restart()
    ...
    STUCK_POSITION_TOLERANCE = (
        1e-4  # treat positions within this as identical (frozen-log case)
    )
    ...
    def _drone_is_stuck(self, position: List[float]) -> bool:
        ...
        if len(self._last_observed_positions) > self.STUCK_THRESHOLD_STEPS:
            self._last_observed_positions = self._last_observed_positions[
    ...
    def _drone_is_unresponsive(
        self, position: List[float], commanded_speed: float
    ) -> bool:
        ...
        if len(self._unresponsive_history) > self.UNRESPONSIVE_WINDOW:
            self._unresponsive_history = self._unresponsive_history[
                -self.UNRESPONSIVE_WINDOW :
            ]
        if len(self._unresponsive_history) < self.UNRESPONSIVE_WINDOW:
            return False
        ...
        moving_commands = sum(
            1
            for _, s in self._unresponsive_history
    ...
    def step(self, action):
        ...
        if is_dead or is_stuck or is_unresponsive:
            ...
            print(
                f"[EvadePursuers2DParticle] Drone appears {reason} (position={evader_pos}) — restarting"
            )
            self.restart()
            self._reset_stuck_tracker()
            self.truncate_next = True
    ...
    RESTART_TIMEOUT_SECONDS = (
        30  # hard ceiling so cflib's no-timeout open_link can't hang us
    )
    ...
    def restart(self):
        ...
        if self._consecutive_restart_failures >= self.MAX_CONSECUTIVE_RESTART_ATTEMPTS:
            print(
                "[EvadePursuers2DParticle] Skipping restart — too many consecutive failures; "
                "the simulator may need a manual restart"
            )
            return False

        print(
            "[EvadePursuers2DParticle] Auto-restarting drone (no user input required)"
        )
        ...
        if not done_event.wait(timeout=self.RESTART_TIMEOUT_SECONDS):
            print(
                f"[EvadePursuers2DParticle] Restart timed out after {self.RESTART_TIMEOUT_SECONDS}s "
        ...
        print(
            "[EvadePursuers2DParticle] WARNING: drone did not confirm takeoff after restart"
        )
        self._consecutive_restart_failures += 1
        return False
    ...
    def _get_state(self) -> np.ndarray:
        ...
        with self._particle_lock:
            pursuer_snapshot = [
                (p[:], v[:])
                for p, v in zip(self.pursuer_positions, self.pursuer_velocities)
            ]
    ...
    def _is_out_of_task_bounds(self, position: List[float]) -> bool:
        ...
        tolerance = 0.05
        return (
            abs(position[0]) > self.xy_limit + tolerance
            or abs(position[1]) > self.xy_limit + tolerance
        )
    ...
    def grab_frame(self, height: int = 540, width: int = 960) -> np.ndarray:
        ...
        for i, p in enumerate(snap):
            ...
            circle = plt.Circle(
                (p[0], p[1]), self.capture_threshold, color=c, alpha=0.15, zorder=1
            )
        ...
        fig.suptitle(
            f"Particle Evasion (Step {self.steps}) | Caught: {self.caught}",
            fontsize=13,
            y=0.98,
        )
