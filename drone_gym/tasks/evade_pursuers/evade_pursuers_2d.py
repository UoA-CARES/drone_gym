...
class EvadePursuers2D(DroneEnvironment):
    ...
    def __init__(
        ...
        n_pursuers: int = 1,
        pursuer_max_velocity: float = 0.20,
    ):

        pursuer_names = [f"pursuer_{i}" for i in range(n_pursuers)]
        super().__init__(
            use_simulator,
            max_velocity,
            step_time,
            expert_drone_names=pursuer_names,
        )
        ...
        self.pursuer_names = pursuer_names
        ...
        self.spawn_min_distance = (
            1.0  # minimum start distance between evader and each pursuer
        )
        self.spawn_margin = 0.1
        self.pursuer_spawn_margin = (
            0.5  # larger margin to prevent overshoot past boundary during reset
        )
        self.fixed_z = 1.0
        ...
        # The base environment owns these drones, including their URIs,
        # reset lifecycle, battery checks, and shutdown.
        self.pursuer_drones = [self.expert_drones[name] for name in pursuer_names]
        for name in pursuer_names:
            self.reset_positions[name] = [0, 0, self.fixed_z]
    ...
    def _compute_pursuer_velocity(
        ...
        if (pursuer_pos[0] <= -soft_limit and vx < 0) or (
            pursuer_pos[0] >= soft_limit and vx > 0
        ):
            vx = 0.0
        if (pursuer_pos[1] <= -soft_limit and vy < 0) or (
            pursuer_pos[1] >= soft_limit and vy > 0
        ):
            vy = 0.0
    ...
    def _sample_pursuer_target_positions(
    ...
    def _fly_pursuers_to_positions(self, target_positions: List[List[float]]):
        ...
        for i, pursuer in enumerate(self.pursuer_drones):
            if not pursuer.is_flying_event.is_set():
                print(
                    f"[EvadePursuers2D] Pursuer {i} is on the ground — taking off again"
                )
                pursuer.take_off()
                if not pursuer.is_flying_event.wait(timeout=15):
        ...
        for i, pursuer in enumerate(self.pursuer_drones):
            if not pursuer.at_reset_position.wait(timeout=12):
                print(
                    f"[EvadePursuers2D] WARNING: pursuer {i} timed out moving to reset position"
                )

        time.sleep(
            1.5
        )  # let pursuers settle at fixed_z before switching to velocity control
    ...
    def reset(self, training: bool = True):
        ...
        if not self.drone.at_reset_position.wait(timeout=15):
            print(
                "[EvadePursuers2D] WARNING: evader timed out moving to random spawn position"
            )
    ...
    def grab_frame(self, height: int = 540, width: int = 960) -> np.ndarray:
        ...
        fig.suptitle(
            f"Evasion Episode (Step {self.steps}) | Caught: {self.caught}",
            fontsize=13,
            y=0.98,
        )
