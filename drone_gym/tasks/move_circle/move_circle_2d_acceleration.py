...
class MoveCircle2DAcceleration(DroneEnvironment):
    ...
    def __init__(
        ...
        self.angular_velocity = (
            0.08  # Radians per step (adjust for faster/slower movement)
        )
    ...
    def _update_goal_position(self):
        ...
        self.target_acceleration = [
            (self.target_velocity[0] - self.previous_target_velocity[0])
            / (self.step_time),
            (self.target_velocity[1] - self.previous_target_velocity[1])
            / (self.step_time),
            (self.target_velocity[2] - self.previous_target_velocity[2])
            / (self.step_time),
        ]
...
if __name__ == "__main__":
    # quick sanity test
    env = MoveCircle2DAcceleration(use_simulator=1)
    env.reset()
