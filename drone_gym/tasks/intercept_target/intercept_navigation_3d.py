...
class InterceptNavigation3D(DroneEnvironment):
    ...
    def __init__(self, use_simulator: Literal[0, 1], max_velocity: float = 0.25, step_time: float = 0.5,
        ...
        # Pulled from the shared default SimManager — the same one DroneSim uses
        # for its own markers/boundary — so one manager owns everything in the sim.
        # crazyflie_pursuer = CrazyflieBody (real flight). We override its policy
        # with a 3D pure-pursuit (the built-in one is xy-only) via the manager's
        # callable-policy seam. For the MARL variant, swap this policy for a learned
        # one — nothing else changes.
        self.sim_manager = get_default_sim_manager()
        self.goal_marker_name = "rl_intercept_nav_goal"
        interceptor_policy = self.sim_manager.create_policy(
            "callable", fn=self._interceptor_pursuit
        )
        self.interceptor = self.sim_manager.create_agent(
            "crazyflie_pursuer", role="interceptor", policy=interceptor_policy,
            fixed_z=self.fixed_z, use_simulator=use_simulator,
        )
