...
class InterceptTargetLine3D(DroneEnvironment):
    """Reinforcement learning task for drone navigation to intercept a target moving in a straight line in 3d"""
    ...
    def __init__(self, use_simulator: Literal[0,1], max_velocity: float = 0.25, step_time: float = 0.5,
        ...
        self.z_lower = 0.5
        self.z_upper = 1.5
        self.z_max_range = self.z_upper - self.z_lower # Max range for Z distances (up and down)
        self.max_xy_range = self.xy_limit * 2  # Maximum range in x or y direction (for normalizing components)
        self.max_distance = np.sqrt(self.max_xy_range ** 2 + self.max_xy_range ** 2 + self.z_max_range ** 2)  # Maximum distance for normalization 
        self.time_tolerance = 0.15 # tolerance time for calculating travel distance

        # hard coded z limit (dont think this is needed) 
        self.boundary = [self.xy_limit, self.xy_limit, self.z_lower, self.z_upper]
    ...
    def step(self, action):
        ...
        if self.learning:
            ...
            processed_action = [action[0], action[1], action[2]] # Add vz=0 to fit 3D action shape of drone_environment
        else:
            # Exploration phase: convert from [0, 1] to [-1, 1]
            processed_action = [action[0] * 2 - 1, action[1] * 2 - 1, action[2] * 2 - 1] # Add vz=0 to fit 3D action shape of drone_environment
    ...
    def _sample_target_start(self):
        ...
        z = np.random.uniform(self.z_lower + self.target_spawn_margin, self.z_upper - self.target_spawn_margin)
    ...
    def _sample_target_velocity(self):
        ...
        vx = self.target_max_velocity * math.cos(theta) * math.sin(phi)
        vy = self.target_max_velocity * math.sin(theta) * math.sin(phi)
        vz = self.target_max_velocity * math.cos(phi)
    ...
    def _update_target_position(self):
        ...
        if new_sz < self.z_lower or new_sz > self.z_upper:
            self.target_velocity[2] *= -1 # Reverse z velocity
            new_sz = self.target_position[2] + self.target_velocity[2] * self.step_time
    ...
    def _distance_to_target(self, position: List[float]) -> float:
        """Calculate 3D Euclidean distance to target position (x, y only)"""
        return math.sqrt(
    ...
    def sample_action(self):
        ...
        # commented out for 3d movement //action[2] = 0  # Set z-action to 0 for 2D movement
        return action
