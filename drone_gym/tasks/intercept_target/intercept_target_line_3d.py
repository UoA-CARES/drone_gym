...
class InterceptTargetLine3D(DroneEnvironment):
    """Reinforcement learning task for drone navigation to intercept a target
    moving in a straight line in 3d"""
    ...
    def __init__(
        ...
        )  # Max range for Z distances (up and down)
        self.max_xy_range = (
            self.xy_limit * 2
        )  # Maximum range in x or y direction (for normalizing components)
        self.max_distance = np.sqrt(
            self.max_xy_range**2 + self.max_xy_range**2 + self.z_max_range**2
    ...
    def _sample_target_start(self):
        ...
        z = np.random.uniform(
            self.z_lower + self.target_spawn_margin,
            self.z_upper - self.target_spawn_margin,
        )
